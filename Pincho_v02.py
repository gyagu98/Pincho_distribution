...
### expression analysis
salmon_dir = os.path.join(bin_dir, "salmon-1.8.0_linux_x86_64/bin")
kallisto_dir = os.path.join(bin_dir, "kallisto_linux-v0.46.1/kallisto")

### misc
sratools_dir = os.path.join(bin_dir, "sratoolkit.2.10.8-ubuntu64/bin")
...
parser.add_argument('-conf', metavar='FILE', type=str, required=True, help="pincho_v02 c_fig File")
...
if c_fig['blast_type1'] == 'mmseqs':
    custom_blast_dir     = os.path.dirname(c_fig['blast_db1'])
    blast_name           = c_fig['blast_db1']  

if c_fig['blast_db2'] != '0':
    if c_fig['blast_type2'] != 'mmseqs':
...
if c_fig['blast_type2'] == 'mmseqs':
    custom_blast_dir_two     = os.path.dirname(c_fig['blast_db2'])
    blast_name_two           = c_fig['blast_db2']  

if c_fig['blast_db3'] != '0':
    if c_fig['blast_type3'] != 'mmseqs':
...
if c_fig['blast_type3'] == 'mmseqs':
    custom_blast_dir_three     = os.path.dirname(c_fig['blast_db3'])
    blast_name_three           = c_fig['blast_db3']  
...
if c_fig['nthreads_blast'] == '0':
    bcoreamount = int(multiprocessing.cpu_count())
else:
    bcoreamount = int(c_fig['nthreads_blast'])


coreamount = int(tcoreamount/sim_runs)
blast_coreamount = int(bcoreamount/sim_runs)
...
def main():
    ...
    if c_fig['read1'] != '0':
        ...
        os.mkdir(f"{cwd}/Pincho_v02_Working_Folder")
        os.chdir(f"{cwd}/Pincho_v02_Working_Folder")
        os.mkdir(f"{sp_name}")

    if c_fig['SRR'] != '0':
        if c_fig['SRR_folder_names'] == '0':
            cwd = os.getcwd()
            SRR = c_fig['SRR']
            os.mkdir(f"{cwd}/Pincho_v02_Working_Folder")
            os.chdir(f"{cwd}/Pincho_v02_Working_Folder")
    ...
    while(wfolder < tfolders): #while current folder count is less than max number of folders do...
        if c_fig['SRR'] != '0':
            try: SRR_number2
            except NameError: SRR_number2 = None
            if SRR_number2 == None:
                SRR_number2 = 0
            else:
                SRR_number2 = SRR_number2 + 1
            SRR_folders = c_fig['SRR_folder_names'] #FOLDER NAMES
            SRR_counter = len(SRR_folders.split())
            print("\n")
            if run_number <= SRR_counter:
                SRR_fold = SRR_folders.split(" ")[SRR_number2]
                os.mkdir(SRR_fold)
                os.chdir(SRR_fold)
...
def fastq_dump():
    global f_reads, r_reads, SRR_number
    start_time = time.time()
    print("Performing Fasterq Dump...")
    SRR = c_fig['SRR']
    coreamount_mod = int(coreamount / 2)
    try: SRR_number
    except NameError: SRR_number = None
    if SRR_number == None:
        SRR_number = 0
    else:
        SRR_number = SRR_number + 1
    SRR_queue = SRR.split(" ")[SRR_number];
    while len(glob.glob(f"{SRR_queue}*")) == 0:
        subprocess.run(f"rm -r fasterq.tmp*", shell=True) 
        subprocess.run(f"{sratools_dir}/fasterq-dump --split-files -e {coreamount_mod} {SRR_queue}", shell=True)        
    else:
        sleep(0)
    subprocess.run(f"sed -i 's/ /_/g' {SRR_queue}*", shell=True)
    subprocess.run(f"pigz {SRR_queue}*", shell=True)
    if path.exists(os.getcwd() + '/' + SRR_queue + ".fastq.gz"):
        f_reads = (os.getcwd() + '/' + SRR_queue + ".fastq.gz")
        r_reads = ''
    if path.exists(os.getcwd() + '/' + SRR_queue + "_1.fastq.gz"):
        f_reads = (os.getcwd() + '/' + SRR_queue + "_1.fastq.gz")
        r_reads = (os.getcwd() + '/' + SRR_queue + "_2.fastq.gz")
...
def blast():
    ...
    if not contig_file:
        ...
        filename = ntpath.basename(f"{contig_file}")
        subprocess.run(f"{blast_dir}/bin/blast{working_blast_process} -query {contig_file} -db {working_blast_name} -evalue {working_evalue} -max_target_seqs 1 -outfmt '7 std qseqid stitle sscinames staxids' -out [{species_name}]_[{working_blast_name}]_blast_table -num_threads {blast_coreamount}", shell=True)
...
def mmseqs2():
    global contig_file, species_name, blast_file2
    start_time = time.time()
    wd = os.getcwd()
    subprocess.run(f"sed -i 's/ /_/g' {working_blast_name}", shell=True)
    subprocess.run(f"sed -i 's/|/_/g' {working_blast_name}", shell=True)
    working_blast_name_no_stem = Path(working_blast_name).stem
    subprocess.run(f"sed -i '/^$/d' {working_blast_name}", shell=True)
    subprocess.run(f"mmseqs easy-search {contig_file} {working_blast_name} -s 7.5 --max-seqs 1 -e 1.000E-10 --threads {blast_coreamount} [{species_name}]_[{working_blast_name_no_stem}]_mmseqs_table mmseqs2_dir", shell=True)
    blast_file2 = f"[{species_name}]_[{working_blast_name_no_stem}]_mmseqs_table"
    shutil.rmtree("mmseqs2_dir")
    subprocess.run(f"sed -i 's/^/>/' {blast_file2}", shell=True)
    subprocess.run(f"cut -f1-4,11-12 {blast_file2} > t222", shell=True)
    subprocess.run(f"sed 'N;s/\\n/\\t/' {contig_file} > as111", shell=True)    
    subprocess.run(f"sed -i 's/ /_/g' t222", shell=True)
    command = """awk 'FNR==NR{a[$1]=$2;b[$1]=$3;c[$1]=$4;d[$1]=$5;e[$1]=$6;next} ($1 in a) {print $1,a[$1],b[$1],c[$1],d[$1],e[$1],$2}' t222 as111 > as222"""
    subprocess.run(command, shell=True)
    subprocess.run(f"sed 's/ /__/; s/ /__/; s/ /__/; s/ /__/; s/ /__/' as222 > as333", shell=True)
    subprocess.run(f"sed 's/ /\\n/g' as333 > {blast_file2}_blast_results.fasta", shell=True)
    subprocess.run(f"rm -f t222 as111 as222 as333", shell=True) 
    if not os.path.exists(f"{wd}/{species_name}_Pincho_Results_Folder"):
        os.makedirs(f"{wd}/{species_name}_Pincho_Results_Folder")
    shutil.copy2(f"{blast_file2}_blast_results.fasta", f"{species_name}_Pincho_Results_Folder/{species_name}_[{working_blast_name_no_stem}]_mmseqs2_results.fasta")
    with open('Time_Log.txt', 'a') as f:
        f.write("mmseqs2: %s \n" % str(datetime.timedelta(seconds=round(time.time() - start_time))))
        f.close()
...
def salmon1(): 
    ...
    if c_fig['annotation_fasta1'] != '0':
        ...
        annotation_exp = f"{owd}/{blast_file2}_blast_results.fasta"
    if len(glob.glob("salmon1")) >= 1:
        print("salmon1 results folder already present")
    elif not r_reads:
        os.mkdir(f"{owd}/salmon1")
        os.chdir(f"{owd}/salmon1")
        subprocess.run(f"{salmon_dir}/salmon index -t {annotation_exp} -i transcripts_index", shell=True)
        subprocess.run(f"{salmon_dir}/salmon quant quant -i transcripts_index -l A -r {f_reads} --validateMappings -o transcripts_quant -p {coreamount}", shell=True)
        os.chdir(owd)
    else:
        os.mkdir(f"{owd}/salmon1")
        os.chdir(f"{owd}/salmon1")
        subprocess.run(f"{salmon_dir}/salmon index -t {annotation_exp} -i transcripts_index", shell=True)
        subprocess.run(f"{salmon_dir}/salmon quant -i transcripts_index -l A -1 {f_reads} -2 {r_reads} --validateMappings -o transcripts_quant -p {coreamount}", shell=True)
        os.chdir(owd)
    if not os.path.exists(f"{owd}/{species_name}_Pincho_Results_Folder"):
        os.makedirs(f"{owd}/{species_name}_Pincho_Results_Folder")
    shutil.copy2(f"{owd}/salmon1/transcripts_quant/quant.sf", f"{species_name}_Pincho_Results_Folder/{species_name}_salmon1_quant.sf")
    with open('Time_Log.txt', 'a') as f:
        f.write("salmon1: %s \n" % str(datetime.timedelta(seconds=round(time.time() - start_time))))
        f.close()
...
def salmon2(): 
    ...
    if c_fig['annotation_fasta2'] != '0':
        ...
        annotation_exp = f"{owd}/{blast_file2}_blast_results.fasta"
    if len(glob.glob("salmon2")) >= 1:
        print("salmon2 results folder already present")
    elif not r_reads:
        os.mkdir(f"{owd}/salmon2")
        os.chdir(f"{owd}/salmon2")
        subprocess.run(f"{salmon_dir}/salmon index -t {annotation_exp} -i transcripts_index", shell=True)
        subprocess.run(f"{salmon_dir}/salmon quant quant -i transcripts_index -l A -r {f_reads} --validateMappings -o transcripts_quant -p {coreamount}", shell=True)
        os.chdir(owd)
    else:
    ...
    if not os.path.exists(f"{owd}/{species_name}_Pincho_Results_Folder"):
        os.makedirs(f"{owd}/{species_name}_Pincho_Results_Folder")
    shutil.copy2(f"{owd}/salmon2/transcripts_quant/quant.sf", f"{species_name}_Pincho_Results_Folder/{species_name}_salmon2_quant.sf")
    with open('Time_Log.txt', 'a') as f:
        f.write("salmon2: %s \n" % str(datetime.timedelta(seconds=round(time.time() - start_time))))
        f.close()
...
#Non-gunzip'ed reads now supported via the manual read option
#Added Salmon to expression analysis toolset
#Added thread option for blast feature, independent from Pincho's performance option
#NOTE: Pincho must be run with a 12:60 thread:memory ratio
#Added batch processing for SRA accession numbers
#TBA Added explorATE to Pincho...
